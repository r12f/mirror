...
void ExportSection(IPEFile* pFile)
{
    ...
    const UINT32 nSectionCount = pFile->GetSectionCount();
    ...
    if (FAILED(pFile->GetOverlay(&pOverlay)) || nullptr == pOverlay)
    {
...
void ExportExportTable(IPEFile* pFile)
{
    ...
    printf("Export Table:\n");
    const UINT32 nExportFunctionCount = pExportTable->GetFunctionCount();
    for (UINT32 nExportFunctionIndex = 0; nExportFunctionIndex < nExportFunctionCount; ++nExportFunctionIndex)
    {
        ...
        printf("Export Function: Name = %s, Ordinal = %d, RVA = 0x%016I64x\n", pExportFunction->GetName(), pExportFunction->GetOrdinal(),
               pExportFunction->GetRVA());
    }
...
void ExportImportTable(IPEFile* pFile)
{
    ...
    printf("Import Table:\n");
    const UINT32 nImportModuleCount = pImportTable->GetModuleCount();
...
void ExportResourceDirectory(IPEResourceDirectory* pDirectory)
{
    ...
    const UINT32 nEntryCount = pDirectory->GetEntryCount();
...
void ExportExceptionTable(IPEFile* pFile)
{
    ...
    const UINT32 nExceptionHandlerCount = pExceptionTable->GetExceptionHandlerCount();
    ...
    for (UINT32 nExceptionHandlerIndex = 0; nExceptionHandlerIndex < nExceptionHandlerCount; ++nExceptionHandlerIndex)
    {
        ...
        if (FAILED(pExceptionTable->GetExceptionHandlerEntryByIndex(nExceptionHandlerIndex, &pExceptionHandlerEntry)) || nullptr == pExceptionHandlerEntry)
        {
            printf("Entry = NULL\n");
            continue;
        }
        ...
        if (FAILED(pExceptionHandlerEntry->GetExceptionHandler(&pExceptionHandler)) || nullptr == pExceptionHandler)
        {
            printf("Handler = NULL\n");
            continue;
        }
...
void ExportRelocationTable(IPEFile* pFile)
{
    ...
    const UINT32 nRelocationPageCount = pRelocationTable->GetPageCount();
    for (UINT32 nPageIndex = 0; nPageIndex < nRelocationPageCount; ++nPageIndex)
    {
        ...
        const UINT32 nRelocationItemCount = pRelocationPage->GetItemCount();
...
void ExportGlobalPointerTable(IPEFile* pFile)
{
    ...
    if (nullptr == pGlobalPointerTable)
    {
        printf("NULL\n\n");
        return;
    }
    ...
    if (FAILED(pGlobalPointerTable->GetRelatedSection(&pGPRelatedSection)) || nullptr == pGPRelatedSection)
    {
        printf("NULL\n\n");
        return;
    }
...
void ExportImportAddressTable(IPEFile* pFile)
{
    ...
    const UINT32 nImportAddressBlockCount = pImportAddressTable->GetBlockCount();
    for (UINT32 nBlockIndex = 0; nBlockIndex < nImportAddressBlockCount; ++nBlockIndex)
    {
        ...
        const UINT32 nImportAddressItemCount = pImportAddressBlock->GetItemCount();
...
void ExportTlsTable(IPEFile* pFile)
{
    ...
    const UINT32 nCallbackCount = pTlsTable->GetCallbackCount();
...
void ExportLoadConfigTable(IPEFile* pFile)
{
    ...
    printf("DependentLoadFlags = %u\n", pLoadConfigTable->GetFieldDependentLoadFlags());
...
void ExportBoundImportTable(IPEFile* pFile)
{
    ...
    const UINT32 nImportModuleCount = pBoundImportTable->GetBoundImportModuleCount();
    for (UINT32 nImportModuleIndex = 0; nImportModuleIndex < nImportModuleCount; ++nImportModuleIndex)
    {
        ...
        const UINT32 nForwarderCount = pBoundImportModule->GetBoundForwarderCount();
...
void ExportCertificateTable(IPEFile* pFile)
{
    ...
    const UINT32 nCertificateCount = pCertificateTable->GetCertificateCount();
    for (UINT32 nCertificateIndex = 0; nCertificateIndex < nCertificateCount; ++nCertificateIndex)
    {
